...
maxRedSum Empty = 0
maxRedSum p = max includeRoot excludeRoot
  where
    (includeRoot, excludeRoot) = helper p


helper :: BinTree Int -> (Int, Int)
helper Empty = (0, 0)
helper (Node a l r) =
  let (includeL, excludeL) = helper l
      (includeR, excludeR) = helper r
      includeHere = a + excludeL + excludeR
      excludeHere = max includeL excludeL + max includeR excludeR
  in (includeHere, excludeHere)